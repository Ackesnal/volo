...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        if not self.shuffle:
            ...
            x_1 = self.proj_1(self.norm_4(x[:,:x.shape[1]//2,:,:].permute(0,2,3,1)).permute(0,3,1,2))
            x_2 = self.proj_2(self.norm_5(x[:,x.shape[1]//2:,:,:].permute(0,2,3,1)).permute(0,3,1,2))
            x = torch.cat((x_1, x_2), dim=1)  # B, C, H, W
        return x
